...
生产环境启用 `PUBLIC_MODE=true` 后，服务端只接受预设的两个公司直播间及 `dual-room-1/2` 槽位，避免公开接口被滥用为任意视频转码服务。具体地址不在公开仓库中展示。
...
- AI 确认的正式风险可通过企业微信群机器人发送模板卡片；演练、非风险和分析失败不会推送
...
- `WECOM_RISK_WEBHOOK_URL`：企业微信群机器人的完整 webhook URL，必须设为 Render Secret
- `WECOM_RISK_NOTIFY_ENABLED`：是否启用正式风险推送，默认 `true`
- `WECOM_RISK_CARD_URL`：点击风险卡片后打开的监控看板地址，默认公网首页

服务端会自动在 API Base 后拼接 `/chat/completions`。AI 风险分析仅在本地关键词命中后调用；未配置文本模型时不会生成正式 AI 风险，也不会触发企微预警。不要把真实凭证写进 Git 仓库。`render.yaml` 默认使用 Free 规格，部署时不要求绑定付费实例；但免费实例会休眠，而且双路实时转码可能因 CPU/内存不足而卡顿或被重启。需要稳定运行时再手动升级规格。
...
- 企业微信 webhook 只从服务端环境变量读取，不返回浏览器；正式风险通知按房间、原文、命中词和结论做短时去重，并采用超时与有限重试。
